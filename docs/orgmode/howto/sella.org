...
free Hessian every ~nsteps_per_diag~ steps. ~gpu_eigh~ /
~gpu_qr~ / ~gpu_project~ are Sella ~_gpu.py~. ~EigenDevice::Dlpk~
is that factory (vecops / dlpk CUDA), not a second GPU stack.
Opt-out is ~SELLA_DISABLE_GPU~; the size floor is
