#+TITLE: Load molecules through readcon
#+OPTIONS: toc:nil todo:nil num:nil author:nil title:nil
#+STARTUP: content

* Load molecules through readcon

Every molecular frame in this stack is a CON frame.

| Crate | Role |
|---+---|
| [[https://github.com/lode-org/readcon-core][readcon-core]] | CON reader / writer, C ABI, spec v2/v3 |
| readcon-chemfiles (~chemfiles~ feature on readcon-core) | Foreign trajectories (XYZ, PDB, ...) into CON |
| [[https://github.com/lode-org/readcon-db][readcon-db]] | mmap corpus for traces and restarts |
| [[https://github.com/d-SEAMS/linkcell][linkcell]] | Periodic MIC and linked-cell k-nearest |
| [[https://github.com/Luthaf/vesin][vesin]] | Cutoff neighbour lists, when a surface needs them |

rgsaddle does not parse CON itself. Enable ~--features readcon~ and
call ~frame_from_con~ to get a 3N Cartesian, masses, and a
~linkcell::Cell~. Enable ~chemfiles~ for foreign formats through
readcon-core. Enable ~readcon-db~ when the host persists an IRC
or dimer trace. vesin is not a Cargo feature here: a surface that needs a
cutoff pair list depends on vesin itself. rgsaddle does not
grow a second neighbour list.

Minimum-image differences on the band go through
~wrap_difference~ (linkcell). They do not go through a private
3x3 box.
