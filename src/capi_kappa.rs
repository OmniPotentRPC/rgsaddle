...
use crate::{capi::*, kappa::{KappaDimerConfig, kappa_dimer_force}, SaddleError};
...
pub unsafe extern "C" fn rgsaddle_kappa_config_default(config: *mut RgsaddleKappaConfig) -> i32 {
    if config.is_null() { return RGSADDLE_INVALID_PARAMETER; }
    let default=KappaDimerConfig::default();
    unsafe { *config=RgsaddleKappaConfig {
        version: RgsaddleVersion { major:RGSADDLE_ABI_MAJOR,minor:RGSADDLE_ABI_MINOR },
        beta:default.beta, tolerance:default.eigen.tol,
        max_iterations:default.eigen.max_iter as i64,
        krylov_dimension:default.eigen.krylov as i64, eigen_kind:default.eigen.kind as i32,
    }; }
    RGSADDLE_OK
}
...
pub unsafe extern "C" fn rgsaddle_kappa_dimer_force(
    config: *const RgsaddleKappaConfig, n_dof:i64,
    gradient:*const f64, mode:*const f64, n_excluded:i64, excluded:*const f64,
    hessian:Option<RgsaddleHessianFn>, user:*mut c_void, force:*mut f64,
    report:*mut RgsaddleKappaReport,
) -> i32 {
    if config.is_null() || gradient.is_null() || mode.is_null() || force.is_null() ||
        report.is_null() || n_dof<=0 || n_excluded<0 || (n_excluded>0 && excluded.is_null()) {
        return RGSADDLE_INVALID_PARAMETER;
    }
    let Some(hessian)=hessian else { return RGSADDLE_NULL_SURFACE; };
    let cfg=unsafe { &*config };
    if cfg.version.major!=RGSADDLE_ABI_MAJOR { return RGSADDLE_ABI_MISMATCH; }
    ...
    let Some(count)=(n_excluded as usize).checked_mul(n) else { return RGSADDLE_SHAPE; };
    ...
    let excluded_slice=if count==0 { &[] } else { unsafe { slice::from_raw_parts(excluded,count) } };
    let excluded=ArrayView2::from_shape((n_excluded as usize,n),excluded_slice).unwrap();
    let cfg=KappaDimerConfig { beta:cfg.beta, eigen:EigenParams {
        kind, nev:1, krylov:cfg.krylov_dimension as usize,
        max_iter:cfg.max_iterations as usize,tol:cfg.tolerance,
    }};
    let result=kappa_dimer_force(g,m,excluded,|v| {
        let mut out=Array1::zeros(n);
        let status=hessian(user,n_dof,v.as_ptr(),out.as_mut_ptr());
        if status!=0 { return Err(SaddleError::Surface(format!("Hessian callback status {status}"))); }
        Ok(out)
    },&cfg);
    match result {
        Ok(out) => {
            unsafe { slice::from_raw_parts_mut(force,n) }.copy_from_slice(out.force.as_slice().unwrap());
            unsafe { *report=RgsaddleKappaReport {
                version:RgsaddleVersion { major:RGSADDLE_ABI_MAJOR,minor:RGSADDLE_ABI_MINOR },
                kappa:out.kappa,tangent_curvature:out.tangent_curvature,
                gamma_parallel:out.gamma_parallel,gamma_perpendicular:out.gamma_perpendicular,
                residual:out.residual,hessian_actions:out.actions as i64,
