...
extern "C" fn hessian(_: *mut c_void, n:i64, v:*const f64, out:*mut f64) -> i32 {
    ...
    for (i,lambda) in [-4.,2.,6.].iter().enumerate() {
        unsafe { *out.add(i)=lambda * *v.add(i); }
    }
    0
}
extern "C" fn fail(_: *mut c_void, _:i64, _:*const f64, _:*mut f64) -> i32 { 17 }
...
fn c_force_matches_the_contour_definition_and_propagates_errors() {
    unsafe {
        ...
        let g=[3.,0.,0.];
        let m=[1.,0.,0.];
        assert_eq!(rgsaddle_kappa_dimer_force(&cfg,3,g.as_ptr(),m.as_ptr(),0,ptr::null(),
            Some(hessian),ptr::null_mut(),out.as_mut_ptr(),report.as_mut_ptr()),RGSADDLE_OK);
        ...
        assert_eq!(rgsaddle_kappa_dimer_force(&cfg,3,g.as_ptr(),m.as_ptr(),0,ptr::null(),
            Some(fail),ptr::null_mut(),out.as_mut_ptr(),error_report.as_mut_ptr()),RGSADDLE_SURFACE_FAILED);
