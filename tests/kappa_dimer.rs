...
fn constrained_mode_excludes_force_without_a_spurious_zero_eigenvalue() {
    ...
    let out = kappa_dimer_force(
        g.view(),
        array![1., 0., 0.].view(),
        Array2::zeros((0, 3)).view(),
        |v| Ok(h.dot(&v)),
        &KappaDimerConfig::default(),
    )
    .unwrap();
...
fn zero_gradient_does_not_divide_or_evaluate_a_hessian() {
    let out = kappa_dimer_force(
        ...
        array![1., 0., 0.].view(),
        Array2::zeros((0, 3)).view(),
        |_| panic!("stationary force needs no kappa"),
        &KappaDimerConfig::default(),
    )
    .unwrap();
...
fn one_dimension_uses_the_pure_uphill_limit() {
    let out = kappa_dimer_force(
        array![2.].view(),
        array![1.].view(),
        Array2::zeros((0, 1)).view(),
        |_| panic!("no tangent directions"),
        &KappaDimerConfig::default(),
    )
    .unwrap();
...
fn failed_hessian_action_is_returned_without_a_force() {
    let err = kappa_dimer_force(
        array![1., 2., 3.].view(),
        array![1., 0., 0.].view(),
        Array2::zeros((0, 3)).view(),
        |_| Err(SaddleError::Surface("rejected HVP".into())),
        &KappaDimerConfig::default(),
    )
    .unwrap_err();
